...
| 13 | **A cross-origin form POST to `/import` does not work.** It returns **403 "Cross origin request forbidden"**, with or without an `Origin` header. `/paste` is only the GET page that renders the form, and the absence of a CSRF token in that HTML means nothing: Lichess enforces the origin server-side. Corrected after a user hit the 403 in production. An earlier version of this table claimed the form POST "always works", inherited from the premise that a form submission is a navigation rather than XHR so CORS cannot block it. True about CORS, irrelevant here, because this is Lichess's own origin check and not a CORS preflight. **Use `/api/import`**, which is built for other origins and sends `access-control-allow-origin: *`. |
